  ...
  for step_ind in 1:length(array_of_tanking_probabilities)
    ...
    for rep = 1:num_replications
      ...
      for i = 1:num_teams
        ...
        stats[i,elim_ind] = -1 # when team is effectively eliminated (game_ind)
      ...
      if (tank_perc == 0.0)
        ...
        tmp_elim_index[:,2] = stats[nonplayoff_teams, elim_ind] # not using negative, because we will sort high-to-low later
